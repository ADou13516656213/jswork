...
function seckill(){
    ...
    if(remaining > 0){
        ...
        h = parseInt((remaining / 3600) % 24);//计算剩余小时（除以60*60转化为小时，与24取余，获取剩余的小时）
        m = parseInt((remaining / 60) % 60);//计算剩余分钟（除以60转为分钟，与60取余，获取剩余的分钟）
        s = parseInt(remaining % 60);//计算剩余秒（与60取余，获取剩余的秒数）
