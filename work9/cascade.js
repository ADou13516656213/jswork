...
var majjorSelect = document.getElementById('majjorSelect');
collegeSelect.onchange = function(){
    majjorSelect.options.length =0;
    createOption(majjorSelect, majors[collegeSelect.value]);
};
var classSelcet = document.getElementById('classSelcet');
majjorSelect.onchange = function(){
    classSelcet.options.length = 0;
    createOption(classSelcet,classes[collegeSelect.value][majjorSelect.value]);
};
collegeSelect.onchange = function(){
    majjorSelect.options.length = 0;
    createOption(majjorSelect,majors[collegeSelect.value]);
    if (collegeSelect.value >= 0){
        majjorSelect.onchange();
    }else{
        classSelcet.options.length = 0;
    }
};
